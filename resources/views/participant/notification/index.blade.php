...
<div class="max-w-4xl mx-auto text-gray-400 mb-20">
    @forelse ($discussions as $disc)
        <a href="{{ route('participant.notification.show', ['threadKey' => $disc->thread_key]) }}">
            <div class="flex items-center {{ !$disc->all_read ? 'bg-gray-700' : 'bg-gray-800' }} shadow-md p-5 cursor-pointer {{ !$loop->last ? 'border-b border-gray-700' : '' }}">
    ...
    <div id="drawer-example" class="fixed top-0 left-0 z-60 h-screen p-4 overflow-y-auto transition-transform -translate-x-full w-full bg-gray-900" tabindex="-1" aria-labelledby="drawer-label">
        <button type="button" data-drawer-hide="drawer-example" aria-controls="drawer-example" class="text-gray-400 bg-transparent rounded-lg text-sm w-8 h-8 absolute top-2.5 left-2.5 flex items-center justify-center hover:bg-yellow-700 hover:text-white" >
