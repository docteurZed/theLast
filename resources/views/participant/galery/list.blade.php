        ...
        <div class="w-full border rounded-lg shadow-sm bg-gray-800 border-gray-700 overflow-hidden relative user-card">
            ...
            <div class="flex flex-col items-center pb-8 pt-12 relative z-10">
                ...
                <div class="absolute -top-12">
                    <img src="{{ $user->profile_photo ?? asset('images/user.png') }}" alt="Photo de profil" class="w-24 h-24 rounded-full border-4 border-gray-800 shadow-lg object-cover">
                </div>

                <h5 class="mt-4 text-xl font-bold text-white text-center">{{ ucfirst($user->first_name) }} {{ ucfirst($user->name) }}</h5>
                <p class="text-sm text-gray-400 text-center px-4 mt-1">{{ $user->bio }}</p>
                ...
                <div class="grid grid-cols-2 gap-2 mt-6 px-4 w-full">
                    <a href="{{ route('participant.galery.show', ['id' => $user->id]) }}" class="bg-gradient-to-r from-yellow-500 via-yellow-600 to-yellow-800 text-white rounded-lg font-semibold shadow-md hover:opacity-90 transition w-full px-5 py-2.5 text-center">
                        Voir
                    </a>
                    <a href="#" data-modal-target="message-modal-{{ $user->id }}" data-modal-toggle="message-modal-{{ $user->id }}" class="hover:bg-gray-700 border border-gray-700 text-white rounded-lg font-semibold shadow-md hover:opacity-90 transition w-full px-5 py-2.5 text-center">
                        Message
                    </a>
                </div>
            </div>
